...
from ParetoMTL.multimnist.min_norm_solvers import MinNormSolver
...
def train(dataset, base_model, niter, npref, init_weight, pref_idx):
    ...
    with open(path+'/original_result.txt', 'a') as f:
        ref_vec_string = ', '.join(map(str, ref_vec[pref_idx].cpu().numpy()))
    ...
    torch.save(model.model.state_dict(), '/home/ubuntu/workspace/dataset/DANC/Result/Model_PMTL/%s_%s_niter_%d_npref_%d_prefidx_%d_original.pickle'%(dataset, base_model, niter, npref, pref_idx))
