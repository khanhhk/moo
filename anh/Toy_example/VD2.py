...
def f(x, i, ref):
    if i == 1:
        return ((x[0]-1)**2 + (x[0]-x[1])**2)/ref[0]
    if i == 2:
        return ((x[0]-3)**2 + (x[0] - x[1])**2)/ref[1]
...
if __name__ == "__main__":
    test_rays = get_reference_directions("das-dennis", 2, n_partitions=50).astype(
        np.float32)
    ...
    plt.savefig(r'/home/ubuntu/workspace/DANC/Toy_example/images/VD2_1.png')
    plt.show()
